...
slice(mtcars,seq(1,nrow(mtcars),2)) #slice or row_num
...
mtcars %>% group_by(am) %>% summarise(MEANMPG = mean(mpg), MAXWT= max(wt))#renamecolname
mtcars %>% group_by(am, gear) %>% summarise_all(mean)
mtcars %>% group_by(am, gear)%>% summarise_all(c("min", "max"))

#specific columns
mtcars %>% summarise_at(c("mpg", "wt"), mean, na.rm = TRUE)#na.rm-to remove missing values in data
(x=c(5,6,7,NA))
mean(x)
mean(x,na.rm=TRUE)
